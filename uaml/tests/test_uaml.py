...
def test_estimator_mlc():
    # first load data and get training and validation sets
    X, y = make_multilabel_classification(n_samples=200, n_classes=10, n_labels=3)
    X_tr, X_te, y_tr, y_te = train_test_split(
...
def test_plot():
    ...
    cm = plt.cm.viridis
    _, ax = plt.subplots(len(classifiers),3,figsize=(10,10))
    for i, clf in enumerate(classifiers.keys()):
        ...
        clf = UAClassifier(model, 0.8, 500, n_jobs=5, verbose=1)
